...
function render_cmdset() {
  ...
  for keyval in ${vmimage_map}; do
    ...
    filepath=$(find ${abs_dirname}/guestroot.${hypervisor}.${arch} -type f -name "${basename}.*")
    [[ -n "${filepath}" ]] || continue
...
function generate_cmdset() {
  ...
  local basepath=${abs_dirname}/guestroot.${hypervisor}.${arch}/var/lib/wakame-vdc/demo/vdc-manage.d
